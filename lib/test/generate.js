...
describe('Generator', function () {
    describe('convertType', function () {
        it('should parse basic types', function () {
            assert(generate_typescript_interfaces_1.convertType('String', false) === 'string');
        });
        it('should convert ArrayOf types', function () {
            assert(generate_typescript_interfaces_1.convertType('ArrayOf(String)', false) === 'string[]');
            assert(generate_typescript_interfaces_1.convertType('ArrayOf(Integer)', false) === 'number[]');
        });
        it('should convert complicated ArrayOf types', function () {
            assert(generate_typescript_interfaces_1.convertType('ArrayOf(Integer, 2)', false) === '[number, number]');
        });
        it('should convert Nvim handles to special values', function () {
            assert(generate_typescript_interfaces_1.convertType('Buffer', false) === 'BufferHandle');
            assert(generate_typescript_interfaces_1.convertType('Buffer', true) === 'Buffer');
        });
    ...
    describe('generateWrappers', function () {
        ...
        it('should log stuff', function () {
            nvimRequestGenerator_1.generateWrappers({}, metadata);
        });
    });
});
